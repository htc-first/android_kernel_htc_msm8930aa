...
bool pm8921_is_skywork_workaround_enabled(void)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return the_chip->skywork_wa_enabled;
}
...
static void dump_all(int more)
{
	...
	pr_info("V=%d mV, I=%d mA, T=%d C, SoC=%d%%, FCC=%d, id=%d mV,"
			...
			" disable_reverse_boost_check=%d, flag=%d%d%d%d\n",
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	chip->skywork_wa_enabled = pdata->skywork_wa_enabled;
