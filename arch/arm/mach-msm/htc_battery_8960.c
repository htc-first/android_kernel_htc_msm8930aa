...
#define HTC_BATT_PWRSRC_DIS_BIT_ID		(1<<2)
#define HTC_BATT_PWRSRC_DIS_BIT_TMR		(1<<3)
#define HTC_BATT_PWRSRC_DIS_BIT_EOC		(1<<4)
...
#define BATT_RECHARGE_PERCENTAGE        (98)
static int chg_stop_by_eoc;
...
static void batt_worker(struct work_struct *work)
{
	...
	if (htc_batt_info.icharger->is_skywork_workaround_enabled
		&& htc_batt_info.icharger->is_skywork_workaround_enabled() )
		if (chg_stop_by_eoc && htc_batt_info.rep.level_raw <= BATT_RECHARGE_PERCENTAGE)
			chg_stop_by_eoc = 0;
	...
	if (htc_batt_info.rep.charging_source > 0) {
		...
		if (htc_batt_info.icharger->is_skywork_workaround_enabled
			...
			if (htc_batt_info.rep.batt_id == HTC_BATTERY_CELL_ID_UNKNOWN)
				pwrsrc_dis_reason |= HTC_BATT_PWRSRC_DIS_BIT_ID; 
			else
				pwrsrc_dis_reason &= ~HTC_BATT_PWRSRC_DIS_BIT_ID;


			if (htc_batt_info.htc_extension & HTC_EXT_CHG_SAFTY_TIMEOUT)
				pwrsrc_dis_reason |= HTC_BATT_PWRSRC_DIS_BIT_TMR;
			else
				pwrsrc_dis_reason &= ~HTC_BATT_PWRSRC_DIS_BIT_TMR;
		} else {
			if (htc_batt_info.rep.batt_id == HTC_BATTERY_CELL_ID_UNKNOWN)
		...
		if (htc_batt_info.icharger->is_skywork_workaround_enabled
			&& htc_batt_info.icharger->is_skywork_workaround_enabled()) {
			if (chg_stop_by_eoc)
				pwrsrc_dis_reason |= HTC_BATT_PWRSRC_DIS_BIT_EOC;
			else
				pwrsrc_dis_reason &= ~HTC_BATT_PWRSRC_DIS_BIT_EOC;
		}
